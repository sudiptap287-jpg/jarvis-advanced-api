...
from datetime import datetime  # Core datetime handling fixed
...
from sqlalchemy import create_engine, Column, String, Integer, Boolean, Float, Text, LargeBinary, DateTime, event
from contextlib import asynccontextmanager
from sqlalchemy.orm import declarative_base, sessionmaker, Session
from fastapi.responses import HTMLResponse

# External modules (Aapki files se imported)
...
from system_doctor import get_system_health

# --- ENVIRONMENT VARIABLES ---
load_dotenv()
MY_PERMANENT_KEY = os.getenv("JARVIS_SUPER_SECRET_KEY", "jarvis_super_secret_2026_key")
...
async def lifespan(app: FastAPI):
    ...
    # 2. Database mein Permanent API Key check/create
    existing_key = db.query(APIKey).filter(APIKey.key_value == MY_PERMANENT_KEY).first()
    if not existing_key:
        new_key = APIKey(key_value=MY_PERMANENT_KEY, owner="sudipto", is_active=True)
        db.add(new_key)
        db.commit()
        print("🔑 Master API Key successfully synced with Database!")
...
if DATABASE_URL.startswith("sqlite"):
    ...
    @event.listens_for(create_engine(DATABASE_URL, connect_args=connect_args), "connect")
...
# =========================================================================
    # 🩺 ADDED LINE: SYSTEM HEALTH DIAGNOSTICS (Linked to system_doctor.py)
    # =========================================================================
    elif "system health" in command or "diagnose system" in command or "pc status" in command:
        print("💡 [DEBUG] Running full system diagnostics...")
        # get_system_health upar imported hai, hum direct call kar rahe hain
        return get_system_health()

    # =========================================================================
    # 💡 ADDED LINE: ADVANCED BRIGHTNESS WITH NATIVE WINDOWS TOAST NOTIFICATION
    # =========================================================================
        ...
        # 1. Exact percentage check (e.g., "set brightness to 70%")
        if any(char.isdigit() for char in command):
            ...
            if numbers:
                ...
                try:
                    ...
                    send_windows_toast("Jarvis System Control", f"Brightness set to {level}%")
        ...
        # 2. Up / Increase Brightness
            ...
            try:
                ...
                send_windows_toast("Jarvis System Control", f"Brightness increased to {new_level}%")
        ...
        # 3. Down / Decrease Brightness
            ...
            try:
                ...
                sbc.set_brightness(new_level)
                send_windows_toast("Jarvis System Control", f"Brightness decreased to {new_level}%")
                return f"Brightness decreased to {new_level} percent."
            except Exception:
                return "Unable to access display hardware."

    # =========================================================================
    # EXISTING CODE: MATH CALCULATION LOGIC
    # =========================================================================
...
@app.post("/get_answer")
async def get_answer_legacy(prompt: str, db: Session = Depends(get_db)):
    result = get_or_set_cache(db, prompt, my_own_api_logic) 
...
async def internal_batch_handler(payload: List[dict], background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    background_tasks.add_task(process_my_own_api_batch, payload, db)
    return {"api_status": "active", "mode": "local_offline_storage", "count": len(payload)}
...
async def create_api_key(owner_name: str, limit: int = 1000, db: Session = Depends(get_db)):
    new_key_value = secrets.token_hex(16)
    new_entry = APIKey(key_value=new_key_value, owner=owner_name, usage_limit=limit, current_usage=0, is_active=True)
    db.add(new_entry)
    db.commit()
    return {"status": "Key Created", "owner": owner_name, "api_key": new_key_value, "limit": limit}

class PremiumInterpreterEngine:
    def __init__(self):
        self.state = {}
        self.common_fixes = {"math": "import math", "json": "import json", "os": "import os"}
    ...
    def try_auto_fix(self, error_msg, code_str):
        if "is not defined" in error_msg:
            ...
            if missing_var in self.common_fixes:
                return f"{self.common_fixes[missing_var]}\n{code_str}", f"Auto-Fixed: Added missing import for '{missing_var}'"
        return None, "Manual correction needed"
...
class PremiumBillingSystem:
    def __init__(self):
        self.rates = {"code_execution": 0.50, "map_view": 1.00, "audio_sync": 0.10}
    ...
    def process_billing(self, user_id: str, feature: str, db: Session):
        ...
        if user:
            user.credits -= cost 
            new_tx = BillingTransaction(user_id=user_id, amount_deducted=cost, feature_used=feature)
            db.add(new_tx)
            db.commit()
            return True, cost
        return False, 0
...
async def premium_control_center(request: Request, command: str, api_key: str, db: Session = Depends(get_db)):
    ...
    new_task = CodeExecutionTask(code_input=command, result_output=f"[{fix_log}] {output}")
    db.add(new_task)
    db.commit()

    return {"status": "Premium_Active", "fix_status": fix_log, "output": output, "timestamp": datetime.now().strftime("%H:%M:%S")}

async def auto_sync_worker():
    while True:
        db = SessionLocal() 
        try:
            pending_tasks = db.query(DeviceSyncLog).filter(DeviceSyncLog.status == "offline_ready").all()
            if pending_tasks:
                async with httpx.AsyncClient() as client:
                    for task in pending_tasks:
                        try:
                            payload_data = task.data_payload.decode('utf-8', errors='ignore') if isinstance(task.data_payload, bytes) else task.data_payload
                            response = await client.post("https://your-private-api.com/sync", json={"data": payload_data}, timeout=5.0)
